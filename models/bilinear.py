...
from utils.data import image_show, image_objects_show, show_single_feature
...
def crop_bbox_batch(feats, bbox, masks, bbox_to_feats, HH, WW=None, backend='cudnn'):
    """
    Inputs:
    ...
    """
    if backend == 'cudnn':
        return crop_bbox_batch_cudnn(feats, bbox, masks, bbox_to_feats, HH, WW)
    N, C, H, W = feats.size()
    B = bbox.size(0)
    if WW is None: WW = HH
...
def crop_bbox_batch_cudnn(feats, bbox, masks, bbox_to_feats, HH, WW=None):
    N, C, H, W = feats.size()
    B = bbox.size(0)
    if WW is None: WW = HH
    dtype = feats.data.type()

    feats_flat, bbox_flat, masks_flat, all_idx = [], [], [], []
    for i in range(N):
        ...
        masks_flat.append(cur_masks)
        all_idx.append(idx)

    feats_flat = torch.cat(feats_flat, dim=0)
    bbox_flat = torch.cat(bbox_flat, dim=0)
    masks_flat = torch.cat(masks_flat, dim=0)
    crops = crop_bbox(feats_flat, bbox_flat, masks_flat, HH, WW, backend='cudnn')
...
def crop_bbox(feats, bbox, masks, HH, WW=None, backend='cudnn'):
    """
...
def masking_feature(boxes, fm_objs, masks):
    ...
    """
    cls, _ = boxes.size()
    cls, C, M, M = fm_objs.size()  # torch.Size([cls, 512, 8, 8])
    cls, H, W = masks.size()

    # show_single_feature(fm_objs, "fm_objs")

    grid = boxes_to_grid(boxes.view(cls, -1), H, W).float().cuda(device=fm_objs.device)

    img_in = fm_objs.float().view(cls, C, M, M)
    bboxed_feature = F.grid_sample(img_in, grid, mode='bilinear')
    cls, C, S, S = bboxed_feature.size()   # torch.Size([cls, 512, 256, 256])

    # # experiment
    # image_show(bboxed_feature, i, "bboxed_feature")
    # image_objects_show(bboxed_feature)
    # show_single_feature(bboxed_feature, "bboxed_feature")

    masks = masks.unsqueeze(1)
    # print("masks.shape: ", masks.shape)   # torch.Size([cls, 1, 256, 256])

    mask_copy = torch.cat([masks] * C, dim=1)   # torch.Size([cls, 512, 256, 256])
    # points of feature map will be lost setup to 0, since they are out of mask, torch.Size([cls, 512, 256, 256])
    masked_feature = bboxed_feature.masked_fill(mask_copy == 0, value=torch.tensor(0))
    # masked_feature = bboxed_feature * masks

    # experiment
    # image_show(masked_feature, i, "masked_feature")
    # image_objects_show(masked_feature)
    # show_single_feature(masked_feature, "masked_feature")

    return masked_feature


def boxes_to_grid(boxes, H, W):
    """
    Input:
