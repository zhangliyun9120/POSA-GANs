...
class Visualizer():
    def __init__(self, opt, result_save_dir):
        ...
        self.opt = opt
        self.saved = False
        self.img_dir = os.path.join(result_save_dir, 'images')
        ...
        if self.use_html:
            self.web_dir = os.path.join(result_save_dir, 'web')
    ...
    def display_current_results(self, visuals, epoch):
        ...
        if self.use_html and self.saved:  # save images to a html file
            self.saved = True
            for label, image in visuals.items():
                image_numpy = util.tensor2im(image)
